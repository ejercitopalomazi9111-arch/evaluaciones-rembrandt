...
export function ValoresTriada() {
  return (
    <Section id="valores" className="bg-hueso-2">
      <Container>
        <div className="grid gap-10 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.9fr)] lg:gap-16">
          ...
          <ul className="list-none self-start">
            {INSTITUCION.valores.map((v, i) => (
              <li
                key={v.nombre}
                className="border-t border-linea last:border-b last:border-linea"
              >
                <Reveal delay={i * 80}>
                  <div className="py-8 sm:py-10">
                    ...
                    <p className="mt-4 max-w-[62ch] pl-9 text-lg text-tinta-suave">{v.texto}</p>
                  </div>
                </Reveal>
              </li>
            ))}
